<div class="bg-white rounded-xl shadow-none p-0">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Modifier Facture #{{ $invoice->invoice_number }}</h1>
    </div>
    ...
    <form action="{{ route('dashboard.invoice.update', $invoice->id) }}" method="POST">
        ...
        <!-- Lignes de facture -->
        <h2 class="text-lg font-bold text-gray-800 mb-4">Lignes de facturation</h2>
        ...
        <div class="mt-8 flex justify-end gap-3">
             <button type="button" onclick="closeModal()" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium">Annuler</button>
            <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium shadow-sm">Enregistrer</button>
        </div>
    </form>
</div>
...
<script>
    if (typeof itemIndex === 'undefined') {
        var itemIndex = {{ $invoice->items->count() }};
    } else {
        itemIndex = {{ $invoice->items->count() }};
    }

    // Attach functions to window to make them globally accessible in the modal context
    window.addItem = function() {
        ...
        row.innerHTML = `
            ...
            <td class="p-2">
                <input type="number" step="0.01" name="items[${itemIndex}][quantity]" value="1" class="w-full rounded border-gray-300 text-sm text-right qty-input focus:ring-indigo-500 focus:border-indigo-500" required oninput="window.calcRow(this)">
            </td>
            <td class="p-2">
                <input type="number" step="0.01" name="items[${itemIndex}][unit_price]" value="0.00" class="w-full rounded border-gray-300 text-sm text-right price-input focus:ring-indigo-500 focus:border-indigo-500" required oninput="window.calcRow(this)">
            </td>
            <td class="p-2 text-right font-bold text-gray-700 row-total">0.00 €</td>
            <td class="p-2 text-center">
                <button type="button" class="text-red-500 hover:text-red-700" onclick="window.removeRow(this)">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                </button>
            </td>
        `;
