    ...
    <nav class="flex-1 overflow-y-auto px-4 py-4">
        ...
        <div class="space-y-1">
            ...
            <a href="{{ route('dashboard.invoice.index') }}"
                class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg border-l-4 transition-all duration-200 {{ request()->routeIs('dashboard.invoice.*') ? 'text-indigo-600 bg-indigo-50 border-indigo-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 hover:border-gray-200 border-transparent' }}">
                <svg class="w-5 h-5 mr-3 transition-colors duration-200 {{ request()->routeIs('dashboard.invoice.*') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-yellow-500' }}" fill="none"
